...
from PyQt6.QtWidgets import QApplication, QPushButton, QWidget, QGridLayout, QLineEdit
...
class Calculator(QWidget):
    ...
    def calculate(self):
        try:
            expression = self.result_fields.text()
            result = eval(expression)
            self.result_fields.setText(str(result))
        except ZeroDivisionError:
            self.result_fields.setText("Error: Division by zero")
        except Exception as e:
            self.result_fields.setText(str(e))
